...
import com.clouidio.play.module.auth.exceptions.AccessTokenException;
import com.clouidio.play.module.auth.exceptions.AuthException;
import com.clouidio.play.module.auth.providers.oauth1.OAuth1AuthProvider;

public class LinkedinAuthProvider extends OAuth1AuthProvider<LinkedinAuthUser, LinkedinAuthInfo> {
	...
	protected LinkedinAuthUser transform(final LinkedinAuthInfo info)
		...
		// final Promise<play.api.libs.ws.Response> promise = WS.url(url).sign(op).get();
		// final Promise<play.api.libs.ws.Response> promiseEmail = WS.url(urlEmail).sign(op).get();
